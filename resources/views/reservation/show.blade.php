            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="mb-6">
                        ...
                        <p class="text-lg"><strong>{{ __('Adresse:') }}</strong> {{ $spectacle->salle->adresse }}</p>
                        <p class="text-lg"><strong>{{ __('Capacité totale:') }}</strong> {{ $spectacle->salle->capacite }} {{ __('personnes') }}</p>
                        <p class="text-lg"><strong>{{ __('Date:') }}</strong> {{ \Carbon\Carbon::parse($spectacle->date_spectacle)->format('d/m/Y') }}</p>
                        <p class="text-lg"><strong>{{ __('Heure:') }}</strong> {{ \Carbon\Carbon::parse($spectacle->heure_spectacle)->format('H:i') }}</p>
                        <p class="text-lg"><strong>{{ __('Prix:') }}</strong> {{ number_format($spectacle->prix, 2) }} €</p>
                        <p class="text-lg"><strong>{{ __('Places restantes:') }}</strong> {{ $remainingPlaces }}</p>
                    </div>

                    <div class="mt-6">
                        <h4 class="text-xl font-semibold mb-4">{{ __('Réserver des places') }}</h4>
